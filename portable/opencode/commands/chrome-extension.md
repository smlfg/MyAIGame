...
> **Note:** The Chrome Browser Extension is Claude Code-specific.
> All `~/.claude/` paths below are intentional -- they reference Claude Code's installation.
> This command is useful for debugging the extension even from within OpenCode.

Pruefe den Status der Claude Code <-> Chrome Browser Extension.

## System-Check

Fuehre diese Checks parallel aus:
...
- **NM Host JSON fehlt** -> `claude --chrome` ausfuehren
...
- Extension kommuniziert ueber Chrome Native Messaging Protocol
